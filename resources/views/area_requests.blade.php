            ...
            <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
                <div class="sidebar-sticky pt-3">
                    <ul class="nav flex-column">
                        <li class="nav-item">
                            <a class="nav-link rounded" href="{{ route('index') }}">
                    ...
                    <table class="table table-striped table-sm">
                        ...
                        <tbody>
                            @foreach ($requests as $request)
                                <tr>
                                    <td>{{ $request->email }}</td>
                                    <td>{{ $request->status == 0 ? 'Inativo' : 'Ativo' }}</td>
                                    @if ($request->status == false)
                                        <td class="d-flex">
                                            <form action="{{ route('unlock', ['id' => $request->id]) }}">
                                                <button class="btn btn-success"><i class="bi bi-unlock-fill"></i></button>
                                            </form>
                                        </td>
                                    @else
                                    <td class="d-flex">
                                        <form action="{{ route('block', ['id' => $request->id]) }}">
                                            <button class="btn btn-danger"><i class="bi bi-lock-fill"></i></button>
                                        </form>
                                    </td>
                                    @endif
                                </tr>
